...
@forelse($rows as $r)
    @php
        ...
        $serieFolio = trim($serie . ' ' . $folio) !== '' ? ($serie . '-' . $folio) : ('#' . $r->id);
        ...
        $total = (float)($r->total_pagos ?? 0);

        $fecha = $r->fecha_pago ?? $r->fecha ?? $r->created_at ?? null;
        $fechaTxt = $fecha ? \Carbon\Carbon::parse($fecha)->format('d/m/Y H:i') : '—';

        $haystack = Str::lower(trim(implode(' ', [
            $serie, $folio, $serieFolio, $razon, $rfc, $uuid, $estatus,
    ...
    <tr data-search="{{ $haystack }}">
        ...
        <td class="px-4 py-3">
            Complemento de pago
            <div class="text-xs text-gray-500">Tipo CFDI: P</div>
        </td>
        ...
        <td class="px-4 py-3">
            <div class="flex items-center justify-end gap-1">
                <a href="{{ route('complementos.ver', $r->id) }}"
                ...
                <a href="{{ route('complementos.xml', $r->id) }}"
                   title="Descargar XML"
                ...
                @if(!empty($r->pdf))
                    <a href="{{ route('complementos.pdf', $r->id) }}"
                       title="Descargar PDF"
                       class="inline-flex items-center justify-center w-9 h-9 rounded-lg border bg-white text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-300">
...
@empty
    <tr>
        <td colspan="7" class="px-4 py-6 text-center text-gray-500">No hay complementos.</td>
    </tr>
@endforelse
